...
export async function processPostData(props, from) {
  ...
  if (allPosts && allPosts.length > 0 && props.post) {
    const index = allPosts.indexOf(props.post)
    props.prev = index >= 0 ? allPosts.slice(index - 1, index)[0] ?? allPosts.slice(-1)[0] : allPosts.slice(-1)[0]
    props.next = index >= 0 ? allPosts.slice(index + 1, index + 2)[0] ?? allPosts[0] : allPosts[0]
